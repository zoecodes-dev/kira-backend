...
def _supplier_id_for(user) -> str | None:
    ...
    원청 계정(admin·owner_*)은 매핑이 없어 None.
    """
...
# [정리] POST /auth/logout(무상태 JWT라 클라이언트가 토큰만 지우면 됨), GET /users/{user_id},
# GET /approval-chain(결재선, 미구현 결재 기능용)은 프론트 호출이 전혀 없어 제거됨(2026-07 전수조사).
