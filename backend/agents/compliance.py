...
CARBON_COMPLIANCE_SYSTEM_PROMPT = (
    ...
    "reasoning 작성 규칙 (원청 담당자가 화면에서 그대로 읽는 보고서 본문입니다):\n"
    "- 전문적이면서 사용자 친화적인 비즈니스 경어체(~합니다)로 작성하세요. 딱딱한 문어체(~한다, ~하회한다, ~판정한다)는 금지합니다.\n"
    "- 흐름: ① 제출된 수치 제시 → ② 조항 기준 대비 평가 → ③ 에너지원 등 보조 근거로 신뢰성 보강 → ④ 신고 의무 이행 여부 확인 → ⑤ 최종 판정 문장으로 마무리.\n"
    "- 마크다운(**굵게**, 목록 기호 등) 없이 순수 문장으로만 작성하세요. 판정 강조는 화면의 뱃지가 담당합니다.\n"
    "- 의미 단락이 바뀌는 지점(수치 제시·기준 평가 / 보조 근거 / 의무 이행·최종 판정)마다 문단을 나누고, "
    "문단 사이는 빈 줄로 구분하세요(JSON 문자열 안에서는 \\n\\n 이스케이프로 표기).\n"
    "- pass 판정 예시 어투: \"제출된 협력사의 탄소집약도는 2.34 kgCO2eq/kWh입니다. 이는 EU 규정상 경고 등급 기준(75 kgCO2eq/kWh 초과)과 "
    "Class A 인증 불가 기준(100 kgCO2eq/kWh 초과)을 모두 안전하게 밑도는 우수한 수치입니다.\\n\\n또한 사용된 에너지원이 재생에너지(renewable)로 "
    "확인되어 낮은 탄소 배출량에 대한 신뢰성이 입증됩니다.\\n\\n관련 법령(Art.7)에 따른 탄소발자국 신고 의무를 정상적으로 이행하였으며, "
    "누락 사항이 없으므로 현재 규제 기준 상 '위반 사항 없음(규제 준수)'으로 판정합니다.\"\n"
    "- violation/warning도 같은 경어체로, 초과·미비 항목과 필요한 조치를 명확하고 정중하게 안내하세요."
)
...
SAQ_COMPLIANCE_SYSTEM_PROMPT = (
    ...
    '"clause_text": "<인용한 조항 원문 발췌>", '
    '"reasoning": "<한국어 근거 설명 — 어떤 SAQ 항목이 어느 조항 리스크에 해당하는지, 비즈니스 경어체>"}\n'
    "규칙:\n"
    "- context에 없는 규정/조항은 절대 인용하지 마세요(환각 금지). context가 비면 verdict='warning', citation=''.\n"
    ...
    "- citation은 context의 조항 표기를 그대로 사용하세요.\n"
    "reasoning 작성 규칙 (원청 담당자가 화면에서 그대로 읽는 보고서 본문입니다):\n"
    "- 전문적이면서 사용자 친화적인 비즈니스 경어체(~합니다)로 작성하세요. 딱딱한 문어체(~한다, ~해당한다, ~판정한다)는 금지합니다.\n"
    "- 흐름: ① 제출된 SAQ 핵심 항목(고충처리·아동/강제노동·인증 등) 요약 → ② 조항 기준 대비 평가 → ③ 리스크 또는 충족 근거 설명 → ④ 최종 판정 문장으로 마무리.\n"
    "- 마크다운(**굵게**, 목록 기호 등) 없이 순수 문장으로만 작성하세요. 판정 강조는 화면의 뱃지가 담당합니다.\n"
    "- 의미 단락이 바뀌는 지점(SAQ 항목 요약 / 조항 기준 평가 / 리스크·충족 근거 / 최종 판정)마다 문단을 나누고, "
    "문단 사이는 빈 줄로 구분하세요(JSON 문자열 안에서는 \\n\\n 이스케이프로 표기).\n"
    "- pass 판정 예시 어투: \"제출된 자가진단 결과, 고충처리 메커니즘이 운영 중이며 아동노동·강제노동 관련 징후가 확인되지 않았습니다. "
    "이는 CSDDD에서 요구하는 인권·환경 실사 의무를 충족하는 수준입니다.\\n\\n또한 ISO 45001 등 안전보건 인증이 확인되어 관리체계의 신뢰성이 "
    "입증됩니다.\\n\\n따라서 현재 실사 기준 상 '실사 요건 충족'으로 판정합니다.\"\n"
    "- violation/warning도 같은 경어체로, 어떤 항목이 어느 조항의 리스크에 해당하는지와 필요한 보완 조치를 명확하고 정중하게 안내하세요."
)
